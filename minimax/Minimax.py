...
class Minimax:
    def __init__(self, max_depth: int, queen_value: float = 3) -> None:
    ...
    def run_minimax(self, game_state: Game, depth: int = None,
        ...
        if is_maximizing_player:
            ...
            for sequence in game_state.valid_moves:
                ...
                # print(f'Depth:{depth - 1}. Eval: {value}')
                # print(f'Trying sequence: {sequence}, eval = {value}')
                if max_eval <= value:
                    max_eval = value
    ...
    def run_minimax_with_branch_pruning(self, game_state: Game, depth: int = None,
                                        alpha: float = float('-inf'), beta: float = float('inf'),
                                        is_maximizing_player: bool = False):
        if depth is None:
            depth = self.max_depth
